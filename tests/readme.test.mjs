...
test("README explains local operation, V2 views, engineering workflow, and security limits", () => {
  ...
  for (const command of ["npm install", "npm run dev", "npm start", "npm run build", "npm run release", "npm test"]) {
  ...
  assert.doesNotMatch(readme, /最多一次查询 20 个 AT/u);
  assert.match(readme, /默认并发 10/u);
